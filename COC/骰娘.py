            ...
            # 角色卡管理
                ...
                if command[4:7] == "new":
                    ...
                    # 未输入名字
                        ...
                        if playershell:
                            playershell.close()
                        playershell = open(
                            ".\\player\\NewPlayerShell.txt", mode="w", encoding="UTF-8"
                        )
                        playershell.write("#Default\n")
                        playershell.close()
                        playershell = open(
                            ".\\player\\NewPlayerShell.txt", mode="r+", encoding="UTF-8"
                        )
                        name = "未命名"
                        # 写入记录
                        if logged == 1:
                ...
                # 角色卡改名
                    ...
                    if command[7:]:
                        ...
                        playershell.close()
                        lname = command[7:]
                        os.rename(".\\player\\" + fname, ".\\player\\" + lname + ".txt")
                        playershell = open(
                        ...
                        try:
                            ...
                            for line in playershell.readlines():
                                if line.find(command[9:]) >= 0:
                                    stsplit = line.split(":")
                                    print(
                                        "%s %s的值为%s" % (name, stsplit[0], stsplit[1]),
                                        end="",
                                    )
                                    # 写入记录
                                    if logged == 1:
                        ...
                        try:
                            ...
                            for line in playershell.readlines():
                                if line[0] == "#":
                                    ...
                                    stsplit = line.split(":")
                                    print("%s的值为%s" % (stsplit[0], stsplit[1]), end="")
                            # 写入记录
                            if logged == 1:
                                record.write(
                                    "%s %s 输入指令 %s\n"
